...
def ddim_train(dataset_name, noise_name):
    def preprocess_image(data):
        ...
        return tf.clip_by_value((image - 127.5)/127.5, -1.0, 1.0)
    ...
    model.save_weights('./saved_models/ddim/{}/{}_with_<{}>_with_{}epochs'.format(dataset_name,dataset_name, noise_name, num_epochs))
...
def ddim_inference(dataset_name, noise_name, num_imgs):
    def sinusoidal_embedding(x):
        embedding_min_frequency = 1.0
        frequencies = tf.exp(
            tf.linspace(
    ...
    class DiffusionModel(keras.Model):
        ...
        def denormalize(self, images):
            # convert the pixel values back to -1-1 range
            images = self.normalizer.mean + images * self.normalizer.variance**0.5
...
#             generated_images = self.denormalize(generated_images)
    ...
    model = DiffusionModel(image_size, widths, block_depth)
    model.load_weights('./saved_models/ddim/{}/{}_with_<{}>_with_{}epochs'.format(dataset_name,dataset_name, noise_name, num_epochs))
#     model.plot_images()
